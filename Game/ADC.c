...
void ADC0_SS2_Init(uint8_t channel, uint16_t mode, uint8_t temp_en)
{
    ...
    while (ADC0_ACTSS_R & ADC_ACTSS_BUSY)
        ;
    ADC0_ACTSS_R &= ~ADC_ACTSS_ASEN2;  // Disable sequencer 2 before configuration

    //2. Configure the trigger event for the sample sequencer in the ADCEMUX register.
    //Use the defines to use a certain trigger, we clear the mask first before setting the mode
    ADC0_EMUX_R &= ~ADC_EMUX_EM2_ALWAYS;
    ADC0_EMUX_R |= mode;  // Set the mode (e.g., software trigger or other mode)

    //3. For each sample in the sample sequence, configure the corresponding input source in the
    //ADCSSMUXn register.
    ADC0_SSMUX2_R |= channel;  // Set the input channel for SS2
    ...
    //Configure the Temperature, Enable Raw Interrupt, Enable End of sequence, Disable differential input
    if (temp_en == ENABLE)
    {
        ADC0_SSCTL2_R |= ADC_SSCTL2_TS0;  // Enable temperature sensor (if temp_en is ENABLE)
    }
    else
    {
        ADC0_SSCTL2_R &= ~(ADC_SSCTL2_TS0);  // Disable temperature sensor if not required
    }
    ADC0_SSCTL2_R |= ADC_SSCTL2_IE0;  // Enable interrupt (if used)
    ADC0_SSCTL2_R |= ADC_SSCTL2_END0;  // Set END bit to indicate the last sample
    ADC0_SSCTL2_R &= ~(ADC_SSCTL2_D0);  // Disable differential input

    //6. If interrupts are to be used, set the corresponding MASK bit in the ADCIM register.
    //Disable the ISR
    ADC0_IM_R &= ~(ADC_IM_MASK2);  // Disable interrupt for SS2 (if you don't need interrupt handling)
    
    //7. Enable the sample sequencer logic by setting the corresponding ASENn bit in the ADCACTSS
    //register.
    ADC0_ACTSS_R |= ADC_ACTSS_ASEN2;  // Enable sequencer 2 after configuration
    return;
}


uint16_t ADC0_SS2_Read(void)
{
    uint16_t result;

    // Trigger the sample sequencer 2 (SS2)
    ADC0_PSSI_R |= ADC_PSSI_SS2;

    // Poll for the raw interrupt flag (this flag will be set when conversion is complete)
    while (!(ADC0_RIS_R & ADC_RIS_INR2))
        ;

    // Read the result from the SS2 FIFO
    result = ADC0_SSFIFO2_R & ADC_SSFIFO2_DATA_M;

    // Clear the interrupt flag before returning the value
