...
void UART_Init(void) {
    ...
    UART0_LCRH_R = (UART_LCRH_WLEN_8 | UART_LCRH_FEN); // 8-bit word length, FIFOs
    UART0_CTL_R |= UART_CTL_UARTEN;       // enable UART

    // Configure PA0 as RX (receive) only
    GPIO_PORTA_AFSEL_R |= 0x01;           // enable alt funct on PA0 (RX)
    GPIO_PORTA_DEN_R |= 0x01;             // enable digital I/O on PA0 (RX)
    GPIO_PORTA_PCTL_R &= ~0x0000000F;     // Clear PCTL bits for PA0
    GPIO_PORTA_PCTL_R |= 0x00000001;      // Set PA0 as UART0 RX
    GPIO_PORTA_AMSEL_R &= ~0x01;          // disable analog functionality on PA0

    // Disable PA1 (TX) functionality by clearing its PCTL bits
    GPIO_PORTA_AFSEL_R &= ~0x02;          // disable alt funct on PA1 (TX)
    GPIO_PORTA_DEN_R &= ~0x02;            // disable digital I/O on PA1 (TX)
    GPIO_PORTA_PCTL_R &= ~0x000000F0;     // Clear PCTL bits for PA1
